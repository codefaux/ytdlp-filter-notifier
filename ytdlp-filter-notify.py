...
def get_latest_videos(channel_url, playlist_end=None):
    ...
    try:
        ...
        return data.get("entries", [])[::-1], data.get("channel") or data.get(
            "title"
        ...
        if not using_netrc:
            print(
                f"{ANSI_YELLOW}Tip: Create a netrc file at {netrc_file} if the video requires login.{ANSI_RESET}"
            )
        return [], None
...
def get_video_upload_date(video_url):
    ...
    if result.returncode != 0:
        print(
            f"{ANSI_RED}yt-dlp error while fetching metadata:{ANSI_RESET} {result.stderr.strip()}"
        )
        if not using_netrc:
            print(
                f"{ANSI_YELLOW}Tip: Create a netrc file at {netrc_file} if the video requires login.{ANSI_RESET}"
...
def preview_recent_videos(
    ...
    for video in videos:
        ...
        if reason == "Matched" and not skip_result:
            ...
            if (
                "short" in reason.lower() or "long" in reason.lower()
            ) and not skip_result:
    ...
    while True:
        if input("Filter by title includes? (y/n): ").strip().lower() == "y":
            ...
            criteria["title_include"] = (
                [word.strip() for word in includes.split(",")] if includes else []
            )
        ...
        if input("Filter by title excludes? (y/n): ").strip().lower() == "y":
            ...
            criteria["title_exclude"] = (
                [word.strip() for word in excludes.split(",")] if excludes else []
            )
...
def interactive_edit_channel(channels_file):
    ...
    videos, discarded = preview_recent_videos(
        url, criteria, playlist_end, current_regex
    )
    print_channel_settings(channel)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Interval in hours to repeat run mode. Default off.",
    )
    parser.add_argument(
        "--suppress-skip-msgs",
        action="store_true",
        help="Suppress not matched/already-seen video messages.",
    )
    parser.add_argument(
        "--seen-during-dry-run",
        action="store_true",
        help="Mark videos as seen during dry-run.",
    )
    parser.add_argument(
        "--disable-telegram-dispatch",
        action="store_true",
        help="Disable Telegram message dispatch.",
    )
    parser.add_argument(
        "--disable-webhook-dispatch",
        action="store_true",
        help="Disable Webhook message dispatch.",
    )
    ...
    if not using_netrc:
        print(
            f"{ANSI_YELLOW}Tip: Create a netrc file at "
            f"{netrc_file} if a platform (eg. nebula) requires login.{ANSI_RESET}"
        )
    ...
    if args.mode in ("run", "dry-run"):
        while True:
            run_all_channels(
                channels_file,
                dry_run=dry_run,
                suppress_skip_msgs=args.suppress_skip_msgs,
                seen_during_dry_run=args.seen_during_dry_run,
            )
            process_message_queue()
            if args.interval_hours <= 0:
                break
            print(
                f"{ANSI_BLUE}Sleeping for {args.interval_hours} hours before next scan...{ANSI_RESET}"
            )
